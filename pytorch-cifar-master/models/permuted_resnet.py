...
class PermBasicBlock_Weighted(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1):
        super(PermBasicBlock_Weighted, self).__init__()

        ################################################
        self.num_channels_permuted = 10
        ################################################

        self.num_filters1 = planes
        self.num_filters2 = planes
        ...
        self.dwconv2 = nn.Conv2d(self.planes, self.planes * self.planes, kernel_size=3, stride=1, padding=1, bias=False, groups=self.planes)
    ...
    def forward(self, x):
        out = self.permute(self.dwconv1(x), self.in_planes, self.planes, self.num_channels_permuted)
        pos = [i for i in range(1, self.num_filters1 + 1)]
        ind = np.argsort(np.array(pos * (int)(out.shape[1] / self.num_filters1)))
        out = out[:, ind, :, :]
        out = F.relu(self.bn1(self.onexone1(out)))

        out = self.permute(self.dwconv2(out), self.planes, self.planes, self.num_channels_permuted)
        pos = [i for i in range(1, self.num_filters2 + 1)]
        ind = np.argsort(np.array(pos * (int)(out.shape[1] / self.num_filters2)))
        out = out[:, ind, :, :]
        out = self.bn2(self.onexone2(out))
...
class PermResNet_Weighted(nn.Module):
    def __init__(self, block, num_blocks, num_classes=100):
        super(PermResNet_Weighted, self).__init__()
        self.in_planes = 16
        ...
        self.layer2 = self._make_layer(PermBasicBlock_Weighted, 32, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(PermBasicBlock_Weighted, 64, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(PermBasicBlock_Weighted, 128, num_blocks[3], stride=2)
        ################################################################################################
...
def PermResNet18_Weighted():
    return PermResNet_Weighted(BasicBlock, [2, 2, 2, 2])
