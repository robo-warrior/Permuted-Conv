...
class Net(nn.Module):
    def __init__(self):
        ...
        self.depthwise3 = nn.Conv2d(self.channelsafter2, self.n_in3 * self.kernels_layer3, kernel_size=self.filter_size3, groups=self.n_in3)
        ...
        # # pool3
        # self.maxpoolscale3 = 2
        # self.pool3 = nn.MaxPool2d(self.maxpoolscale3, self.maxpoolscale3)
        # self.heightwidthafterpool3 = (int)(self.heightwidthafter3 / 2)
...
for epoch in range(10001):  # loop over the dataset multiple times
    ...
    experiment.log_figure(figure=plt, figure_name='loss_per_epoch_plot_baseline', overwrite=True)
    plt.close()
    ...
    if (epoch % 5 == 0):
        ...
        plt.title('Loss plot')
        plt.legend()
        plt.savefig("./loss_plot_baseline.png", format='png')
        experiment.log_figure(figure=plt, figure_name='loss_plot_baseline', overwrite=True)
        plt.close()
        ...
        plt.legend()
        plt.savefig("./accuracy_plot_baseline.png", format='png')
        experiment.log_figure(figure=plt, figure_name='accuracy_plot_baseline', overwrite=True)
        plt.close()
